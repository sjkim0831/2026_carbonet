...
export function CompanyApproveMigrationPage() {
  ...
  const [page, setPage] = useState<CompanyApprovePagePayload | null>(null);
  ...
  async function load(next?: { pageIndex?: number; searchKeyword?: string; sbscrbSttus?: string; result?: string; }) {
    const [sessionPayload, pagePayload] = await Promise.all([
      session ? Promise.resolve(session) : fetchFrontendSession(),
      fetchCompanyApprovePage(next)
    ]);
  ...
  async function handleAction(action: string, insttId?: string) {
    ...
    setError("");
    try {
      const result = await submitCompanyApproveAction(session, {
        action,
        insttId,
        selectedIds: insttId ? undefined : selectedIds
      });
      await load({ searchKeyword, sbscrbSttus: status, result: String(result.result || "") });
    } catch (err) {
  ...
  return (
    <main className="app-shell">
      ...
      <CanView allowed={!!page?.canViewCompanyApprove} fallback={<section className="panel"><p className="state-text">회원사 승인 화면을 볼 권한이 없습니다.</p></section>}>
        <section className="panel">
          ...
          <div className="toolbar-actions">
            <PermissionButton allowed={!!page?.canUseCompanyApproveAction && selectedIds.length > 0} className="primary-button" onClick={() => handleAction("batch_approve")} reason="전체 관리자만 승인할 수 있습니다." type="button">선택 승인</PermissionButton>
            <PermissionButton allowed={!!page?.canUseCompanyApproveAction && selectedIds.length > 0} className="primary-button" onClick={() => handleAction("batch_reject")} reason="전체 관리자만 반려할 수 있습니다." type="button">선택 반려</PermissionButton>
          </div>
        </section>
        <section className="panel">
                ...
                {(page?.approvalRows || []).length === 0 ? (
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td>{String(row.businessNumber || "-")}</td>
                      <td>{String(row.representativeName || "-")}</td>
                      <td>{String(row.statusLabel || "-")}</td>
                      <td>
                        <div className="toolbar-actions">
                          <PermissionButton allowed={!!page?.canUseCompanyApproveAction} className="primary-button" onClick={() => handleAction("approve", id)} reason="전체 관리자만 승인할 수 있습니다." type="button">승인</PermissionButton>
                          <PermissionButton allowed={!!page?.canUseCompanyApproveAction} className="primary-button" onClick={() => handleAction("reject", id)} reason="전체 관리자만 반려할 수 있습니다." type="button">반려</PermissionButton>
                        </div>
                      </td>
                    </tr>
                  );
