...
이 저장소는 [stable-restart-guide.md](/opt/projects/carbonet-react-migration/docs/operations/stable-restart-guide.md) 기준으로 `systemd` 서비스 운용이 가장 적합하다.
문서 내 `<PROJECT_ROOT>` 기본값은 `PROJECT_PATHS.md`와 `ops/project-paths.sh`를 따른다.
