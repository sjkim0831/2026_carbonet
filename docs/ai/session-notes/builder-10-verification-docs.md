...
- Verified frontend build command: `npm run build` in `/opt/projects/carbonet/frontend`.
- Verified backend package command: `mvn -q -DskipTests package` in `/opt/projects/carbonet`.
