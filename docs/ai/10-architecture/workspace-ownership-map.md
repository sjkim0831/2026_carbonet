...
| --- | --- | --- | --- | --- | --- | --- |
| `/admin/system/codex-request` | `frontend/src/features/codex-provision/CodexProvisionMigrationPage.tsx` | `CodexProvisionPageController.codexProvisionPageData`, `CodexProvisionAdminController.tickets`, `ticketDetail`, `ticketArtifact` | `/login`, `/execute`, `/history`, `/history/{logId}/inspect`, `/history/{logId}/remediate`, `/tickets/{ticketId}/prepare|plan|execute|direct-execute|queue-direct-execute|skip-plan-execute|rollback|reissue|delete` | `CodexProvisionPageController`, `CodexProvisionAdminController`, `SrTicketWorkbenchServiceImpl`, `CodexExecutionAdminService` | mixed: SR JSONL, runner artifacts in filesystem, execution history file, runtime env/config | ticket and runner actions should correlate to audit and runner artifact paths; queue console is the central operator review surface |
...
| `/admin/system/observability` and related log/security routes | `frontend/src/features/observability/ObservabilityMigrationPage.tsx` | page-data endpoints under `AdminObservabilityController` and search APIs under `/api/admin/observability/*` | backup, restore, export, policy save, and query actions inside `AdminObservabilityController` | `AdminObservabilityController`, `AdminObservabilityPageService`, `ObservabilityQueryService` | DB-backed audit and trace records plus operational config tables/files by feature | this is the verification and evidence plane for all workspace families |
