-- Align emission variable/factor definitions with the KTL cement/lime formula reference.
-- Safe for an already-provisioned local DB that used the first skeleton seed.

DELETE FROM emission_input_value
WHERE session_id IN (
    SELECT session_id
    FROM emission_input_session
    WHERE category_id IN (1, 2)
);

DELETE FROM emission_calc_result
WHERE session_id IN (
    SELECT session_id
    FROM emission_input_session
    WHERE category_id IN (1, 2)
...
DELETE FROM emission_input_session
WHERE category_id IN (1, 2);

DELETE FROM emission_variable_def
WHERE category_id IN (1, 2);
...
VALUES
    ...
    (6, 1, 2, 'MD', '재활용되지 않은 CKD 질량', '킬른에 재활용되지 않은 CKD 질량', 'ton', 'NUMBER', 'USER', 'N', 'Y', 20, 'Y'),
    (7, 1, 2, 'CD', 'CKD 원래 탄산염 비율', 'CKD에서 원래 탄산염의 비율', 'ratio', 'NUMBER', 'USER', 'N', 'Y', 30, 'Y'),
    (8, 1, 2, 'FD', 'CKD 소성 비율', 'CKD에서 달성된 소성 비율', 'ratio', 'NUMBER', 'USER', 'N', 'Y', 40, 'Y'),
    ...
    (23, 2, 2, 'LIME_TYPE', '석회 유형', '고칼슘석회, 고토석회(선진국), 고토석회(개도국), 수경성석회 중 입력. 고토석회는 기본 함유량 사용 시 선진국/개도국을 구분', 'text', 'TEXT', 'USER', 'Y', 'Y', 10, 'Y'),
