...
@Slf4j
public class AdminEmissionSurveyDataMenuBootstrapSupport {
    ...
    private static final String MENU_CODE = "A0020112";
    private static final String MENU_NAME_KO = "배출 설문 데이터셋";
    private static final String MENU_NAME_EN = "Emission Survey Datasets";
    private static final String MENU_URL = "/admin/emission/survey-admin-data";
    private static final String MENU_ICON = "database";
    ...
    public void ensureMenu() {
        try {
            ...
            log.info("Emission survey dataset menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision emission survey dataset menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-SURVEY-DATA");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "데이터셋 조회", "View Survey Datasets", "Review account-scoped survey datasets"),
                featureRequest(MENU_CODE + "_LOG", "업로드 로그 조회", "View Upload Logs", "Review upload execution logs and results")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master", MENU_CODE + "_VIEW", MENU_CODE + "_LOG"),
                authorRequest("ROLE_SYSTEM_ADMIN", "시스템 관리자", "System Administrator", MENU_CODE + "_VIEW", MENU_CODE + "_LOG"),
                authorRequest("ROLE_ADMIN", "일반 관리자", "General Administrator", MENU_CODE + "_VIEW", MENU_CODE + "_LOG")
        ));
        return request;
    }
