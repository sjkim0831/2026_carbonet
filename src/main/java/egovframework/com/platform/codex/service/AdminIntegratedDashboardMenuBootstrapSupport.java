...
@Slf4j
public class AdminIntegratedDashboardMenuBootstrapSupport {

    private static final String DOMAIN_CODE = "A007";
    private static final String DOMAIN_NAME = "운영";
    private static final String DOMAIN_NAME_EN = "Operations";
    private static final String GROUP_CODE = "A00701";
    private static final String GROUP_NAME = "대시보드";
    private static final String GROUP_NAME_EN = "Dashboard";
    ...
    public void ensureIntegratedDashboardMenu() {
        try {
            ...
            log.info("Integrated dashboard menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision integrated dashboard menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-INTEGRATED-DASHBOARD");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "대시보드 조회", "View dashboard", "Integrated dashboard page access")
        ));
