...
@Slf4j
public class AdminEmissionSurveyMenuBootstrapSupport {
    ...
    private static final String MENU_NAME_KO = "배출 설문 관리";
    private static final String MENU_NAME_EN = "Emission Survey Management";
    private static final String MENU_URL = "/admin/emission/survey-admin";
    private static final String MENU_ICON = "upload_file";
    ...
    public void ensureMenu() {
        try {
            ...
            log.info("Emission survey admin menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision emission survey admin menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-SURVEY-ADMIN");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "배출 설문 조회", "View Emission Survey", "Open the emission survey management workspace"),
                featureRequest(MENU_CODE + "_UPLOAD", "엑셀 업로드", "Upload Survey Workbook", "Upload and parse the emission survey workbook"),
                featureRequest(MENU_CODE + "_SAVE", "케이스 저장", "Save Survey Cases", "Save local survey case drafts")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master",
                        MENU_CODE + "_VIEW", MENU_CODE + "_UPLOAD", MENU_CODE + "_SAVE"),
