...
@Slf4j
public class AdminEmissionDataHistoryMenuBootstrapSupport {
    ...
    private static final String MENU_CODE = "A0020106";
    private static final String MENU_NAME_KO = "데이터 변경 이력";
    private static final String MENU_NAME_EN = "Data Change History";
    private static final String MENU_URL = "/admin/emission/data_history";
    ...
    public void ensureEmissionDataHistoryMenu() {
        try {
            ...
            log.info("Emission data history menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision emission data history menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-DATA-HISTORY");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "이력 조회", "View history", "Access the data history page"),
                featureRequest(MENU_CODE + "_FILTER", "이력 필터", "Filter history", "Filter history by type, target, and keyword"),
                featureRequest(MENU_CODE + "_DETAIL", "이력 상세", "History detail", "Open linked result or detail traces"),
                featureRequest(MENU_CODE + "_EXPORT", "이력 내보내기", "Export history", "Export history rows for audit and review"),
                featureRequest(MENU_CODE + "_TRACE", "변경 추적", "Change trace", "Trace before and after value changes")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master",
                        MENU_CODE + "_VIEW", MENU_CODE + "_FILTER", MENU_CODE + "_DETAIL", MENU_CODE + "_EXPORT", MENU_CODE + "_TRACE"),
