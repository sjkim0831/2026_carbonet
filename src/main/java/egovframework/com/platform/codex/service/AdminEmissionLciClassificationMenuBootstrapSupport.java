...
@Slf4j
public class AdminEmissionLciClassificationMenuBootstrapSupport {
    ...
    private static final String MENU_NAME_KO = "LCI 분류 관리";
    private static final String MENU_NAME_EN = "LCI Classification Management";
    private static final String MENU_URL = "/admin/emission/lci-classification";
    private static final String MENU_ICON = "schema";
    private static final String ACTOR_ID = "SYSTEM_BOOTSTRAP";
    private static final String SOURCE_VIEW_FEATURE_CODE = "A0020110_VIEW";
    private static final String TARGET_VIEW_FEATURE_CODE = MENU_CODE + "_VIEW";
    ...
    public void ensureMenu() {
        try {
            ...
            syncVisibleAuthorsFromSurveyAdmin();
            log.info("Emission LCI classification menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision emission LCI classification menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-LCI-CLASSIFICATION");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "LCI 분류 조회", "View LCI classifications", "Open the shared LCI classification workspace"),
                featureRequest(MENU_CODE + "_SAVE", "LCI 분류 저장", "Save LCI classifications", "Create and update LCI classification rows"),
                featureRequest(MENU_CODE + "_DELETE", "LCI 분류 삭제", "Delete LCI classifications", "Delete LCI classification rows")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master",
                        MENU_CODE + "_VIEW", MENU_CODE + "_SAVE", MENU_CODE + "_DELETE"),
    ...
    private void syncVisibleAuthorsFromSurveyAdmin() {
        List<String> sourceAuthors = authGroupManageMapper.selectAuthorCodesByFeatureCode(SOURCE_VIEW_FEATURE_CODE);
        int inserted = 0;
        for (String authorCodeValue : sourceAuthors) {
            String authorCode = safe(authorCodeValue);
            if (authorCode.isEmpty()) {
                continue;
            }
            if (authGroupManageMapper.countAuthorFeaturePermission(authorCode, TARGET_VIEW_FEATURE_CODE) > 0) {
                continue;
            }
            Map<String, String> params = new HashMap<>();
            params.put("authorCode", authorCode);
            params.put("featureCode", TARGET_VIEW_FEATURE_CODE);
            authGroupManageMapper.insertAuthorFeatureRelation(params);
            inserted += 1;
        }
        log.info("Emission LCI classification visibility synced from survey admin. sourceFeature={}, targetFeature={}, inserted={}",
                SOURCE_VIEW_FEATURE_CODE, TARGET_VIEW_FEATURE_CODE, inserted);
    }
