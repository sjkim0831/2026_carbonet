...
public class AdminMemberRegisterMenuBootstrapSupport {
    ...
    private static final String MENU_URL = "/admin/member/register";
    private static final String MENU_ICON = "person_add";
    ...
    public void ensureMemberRegisterMenu() {
        try {
    ...
    private CodexProvisionRequest buildRequest() {
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "회원 등록 조회", "View member registration", "Member registration page access"),
                featureRequest(MENU_CODE + "_SAVE", "회원 등록 저장", "Save member registration", "Save new member information")
        ));
        request.setAuthors(Arrays.asList(
                ...
                authorRequest("ROLE_ADMIN", "일반 관리자", "General Administrator", MENU_CODE + "_VIEW", MENU_CODE + "_SAVE")
        ));
        return request;
    }
