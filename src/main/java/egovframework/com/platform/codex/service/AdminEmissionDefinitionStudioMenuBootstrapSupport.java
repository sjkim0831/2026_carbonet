...
@Slf4j
public class AdminEmissionDefinitionStudioMenuBootstrapSupport {
    ...
    private static final String MENU_NAME_KO = "배출 정의 관리";
    private static final String MENU_NAME_EN = "Emission Definition Studio";
    private static final String MENU_URL = "/admin/emission/definition-studio";
    ...
    public void ensureEmissionDefinitionStudioMenu() {
        try {
            ...
            log.info("Emission definition studio menu provisioned. created={}, existing={}, skipped={}",
                    response.getCreatedCount(), response.getExistingCount(), response.getSkippedCount());
        } catch (Exception e) {
            log.error("Failed to provision emission definition studio menu.", e);
        }
    }

    private CodexProvisionRequest buildRequest() {
        CodexProvisionRequest request = new CodexProvisionRequest();
        request.setRequestId("BOOTSTRAP-EMISSION-DEFINITION-STUDIO");
        ...
        request.setFeatures(Arrays.asList(
                featureRequest(MENU_CODE + "_VIEW", "정의 관리 조회", "View definition studio", "Open the emission definition studio"),
                featureRequest(MENU_CODE + "_CATEGORY", "분류 관리", "Manage categories", "Create and edit emission categories"),
                featureRequest(MENU_CODE + "_TIER", "Tier 관리", "Manage tiers", "Create and edit emission tiers"),
                featureRequest(MENU_CODE + "_FORMULA", "수식 관리", "Manage formulas", "Register and update formula metadata"),
                featureRequest(MENU_CODE + "_POLICY", "규칙 관리", "Manage policies", "Manage validation and resolution policies")
        ));
        request.setAuthors(Arrays.asList(
                authorRequest("ROLE_SYSTEM_MASTER", "시스템 마스터", "System Master",
                        MENU_CODE + "_VIEW", MENU_CODE + "_CATEGORY", MENU_CODE + "_TIER", MENU_CODE + "_FORMULA", MENU_CODE + "_POLICY"),
