...
description: Build or extend Carbonet menus, pages, services, mappers, React shell routes, and related DB/menu metadata in this repository. Use when Codex must implement a new Carbonet screen or business function from menu definitions, screen artifacts under `/opt/reference/screen` or `/home/imaneya/workspace/화면설계`, or existing admin/member/home patterns while preserving the current package layout, MyBatis conventions, bilingual routing, and menu-code system.
---

# Carbonet Feature Builder

Implement new Carbonet features by matching the existing project structure first, then mapping the requested menu or screen to menu codes, DB metadata, React shell routes, services, and permissions.
...
2. If the target screen already exists as a React route or React shell-backed page in this repository, treat that implementation as the canonical production source before designing anything.
3. If the target screen exists only as a design HTML or legacy reference artifact, treat that file as a parity reference rather than production code.
4. For React migration work, prefer a parity workflow over redesign:
...
5. If the request came from `/home/imaneya/workspace/화면설계`, read the root source files in this order first:
   - `1. main_home_menu_designed.html`
   - `2. main_home_menu.html`
   - `3. admin_menu_dashboard.html`
   - `4. requirements_gap_dashboard.html`
6. If more detail is needed, trace the same feature in `설계/00_요구사항매핑.txt`, `화면설계서_최종통합.txt`, and the nearest detailed HTML mapping file under `HTML_서비스설계_v8` or `설계HTML_완성본_v8`.
7. Treat the local workspace copy as the primary design source and treat duplicate folders (`html`, `설계HTML`, `설계HTML_완성본_v8`, `화면`) as mirrored outputs unless timestamps or filenames clearly indicate a newer revision.
8. If a screen implies schema, code-table, or naming changes, inspect `DB_설계서_DDL.txt` and the public-data standard-term CSV before inventing table or column names.
9. Confirm whether the feature is `ADMIN` (`AMENU1`, `/admin/...`) or user/home (`HMENU1`, `/home...`, `/mypage`, `/join...`).
10. Check whether the feature is a plain page, a request workflow, or an approval workflow.
11. Trace the closest existing implementation before designing anything new.
12. Implement from the outside in:
   - route and React shell entry contract
   - controller
...
- The active production page shell is `src/main/resources/static/react-shell/index.html`, and page entry assets are served from `src/main/resources/static/react-app/`.
- Prefer React route ids, Spring controller mappings, and bootstrap payload contracts over introducing new server-side templates.
- If a task references legacy templates from docs or design notes, verify that the file still exists in this repository before treating it as canonical.
