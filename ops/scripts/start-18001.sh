...
if [[ ! -f "$JAR_PATH" ]]; then
  echo "[start-18001] missing jar: $JAR_PATH" >&2
  exit 1
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "${EXISTING_PID:-}" ]] && kill -0 "$EXISTING_PID" 2>/dev/null; then
    echo "[start-18001] already running: pid=$EXISTING_PID port=$PORT"
    exit 0
  fi
  rm -f "$PID_FILE"
fi
...
for attempt in $(seq 1 "$START_RETRY_COUNT"); do
  printf '\n[start-18001] %s attempt=%s/%s db=%s log=%s\n' \
  ...
  for _ in $(seq 1 "$STARTUP_WAIT_SECONDS"); do
    ...
    if tail -n 120 "$LOG_FILE" 2>/dev/null | grep -q "Tomcat started on port(s): $PORT"; then
      echo "[start-18001] started: pid=$APP_PID port=$PORT log=$LOG_FILE"
      exit 0
    fi
    sleep 1
  done
  ...
  if ! kill -0 "$APP_PID" 2>/dev/null; then
    ...
    if [[ "$attempt" -lt "$START_RETRY_COUNT" ]]; then
      echo "[start-18001] startup attempt ${attempt}/${START_RETRY_COUNT} exited early. retrying in ${RETRY_DELAY_SECONDS}s..." >&2
      sleep "$RETRY_DELAY_SECONDS"
      continue
    fi
    echo "[start-18001] process exited early. recent log:" >&2
    tail -n 60 "$LOG_FILE" >&2 || true
    exit 1
  fi

  echo "[start-18001] process is running but startup was not confirmed in ${STARTUP_WAIT_SECONDS}s. recent log:" >&2
  tail -n 60 "$LOG_FILE" >&2 || true
  exit 1
done
