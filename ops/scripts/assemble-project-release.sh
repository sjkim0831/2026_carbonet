...
# 1. Build and copy common runtime
echo "[assemble] building common project-runtime..."
mvn -q -pl apps/project-runtime -am -DskipTests clean package
cp "$ROOT_DIR/apps/project-runtime/target/project-runtime.jar" "$RELEASE_BASE/"
...
mkdir -p "$RELEASE_BASE/config" "$RELEASE_BASE/db"
...
# Optional: copy application.yml if exists
...
# 4. Create run script for this release
