...
usage() {
  ...
  echo "  CODEX_BASE_URL           Optional base URL, default: http://localhost:18000"
...
PAYLOAD_SOURCE="$1"
BASE_URL="${2:-${CODEX_BASE_URL:-http://localhost:18000}}"
...
if [ "$LOGIN_CHECK" = "true" ]; then
  ...
  login_response="$(curl -sS -X POST "${BASE_URL%/}/signin/codex/login" "${header_args[@]}")"
  echo "$login_response"
fi

echo "[2/2] Sending provision request..."
response="$(curl -sS -X POST "${BASE_URL%/}/signin/codex/provision" "${header_args[@]}" --data @"$PAYLOAD_FILE")"
echo "$response"
