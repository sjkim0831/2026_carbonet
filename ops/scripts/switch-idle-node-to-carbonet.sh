...
IDLE_REMOTE_ROOT="${IDLE_REMOTE_ROOT:-/opt/projects/carbonet}"
...
UPSTREAM_WRITER="${UPSTREAM_WRITER:-/opt/projects/carbonet/ops/scripts/write-idle-upstream.sh}"
...
main() {
  ...
  run_remote "$IDLE_TARGET" "mkdir -p '$IDLE_REMOTE_ROOT/target' '$IDLE_REMOTE_ROOT/var/logs'"
  copy_remote "$SOURCE_JAR_PATH" "${IDLE_TARGET}:${IDLE_REMOTE_ROOT}/target/carbonet.jar"

  log "render and upload Nomad job"
  local tmp_job
  tmp_job="$(mktemp)"
  TARGET_HOST="$IDLE_HTTP_HOST" REPO_ROOT="$IDLE_REMOTE_ROOT" JAR_PATH="$IDLE_REMOTE_ROOT/target/carbonet.jar" TARGET_PORT="$IDLE_PORT" JOB_NAME="$IDLE_JOB_NAME" \
